...
export default function SettingsModal({ onClose }) {
  ...
  const handlePortalAnalyze = () => {
    // allCourses가 없으면 먼저 로드
    if (allCourses.length === 0) {
      coursesAPI.getAll().then(res => {
        const courses = res.data.courses || [];
        setAllCourses(courses);
        analyzeWithCourses(courses);
      });
    } else {
      analyzeWithCourses(allCourses);
    }
  };

  const analyzeWithCourses = (courses) => {
    ...
    const matched = courses.filter(c => kentech.includes(c.code));
  ...
  // 포털 확인 → importByCodes 호출 (기존 이력에 추가)
  const handlePortalConfirm = async () => {
    const codes = portalPreview.matched.map(c => c.code);
    setSaving(true);
    try {
      await coursesAPI.importByCodes(codes);
      setPortalDone(true);
    } catch {
      alert('서버 오류가 발생했습니다.');
    } finally {
          ...
          {mode === 'portal' && (
            portalDone ? (
              <div className="settings-done">
                <p className="done-msg">✓ {portalPreview?.matched?.length || 0}개 과목이 수강이력에 추가되었습니다.</p>
              ...
              <>
                ...
                <div className="settings-btns">
                  ...
                  <button
                    ...
                    disabled={saving || portalPreview.matched.length === 0}
                  >
                    {saving ? '저장 중...' : `${portalPreview.matched.length}개 추가`}
                  </button>
                </div>
              </>
            ) : (
