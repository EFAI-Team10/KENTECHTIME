...
16. [졸업 요건 로직](#졸업-요건-로직)
17. [모듈별 책임](#모듈별-책임)
18. [트러블슈팅 / 회고](#트러블슈팅--회고)
...
|------|---------|---------------|-----------|
| 박현담 | 81 | `components/Dashboard/`, `lib/server/recommender.js`, `app/api/courses/requirements/`, `contexts/ThemeContext`, `lib/themes.js` | 졸업요건 계산·대시보드, 시간표 추천 알고리즘, 다크모드·9종 색상 테마 |
| 강민기 | 63 | `components/Timetable/`, `app/page.jsx` | 아이디어 제공, 시간표 UI/UX 개선, 내 시간표(복사·저장·확정·다중), 전반 QA |
| 권우성 | 56 | `app/api/auth/`, `app/api/chat/`, `components/Chat/`, `lib/gradeParser.js`, `public/bookmarklet.js` | Next.js 마이그레이션, 구글 OAuth 인증, AI 대화형 시간표 수정, 포털 성적 북마클릿 |
| 구형준 | 4 | 전역 CSS, 프로젝트 구조 | 디자인·스타일 통일, 구조/로직 체계화, QA |

> 커밋 수는 `git shortlog` 기준이며, 디자인·QA 등 일부 작업은 커밋 수로 온전히 드러나지 않습니다.
...
> 개발 기간: 2026.05.24 ~ 2026.06.07

```mermaid
gantt
    title KENTECHTIME 프로젝트 타임라인
    dateFormat  YYYY-MM-DD
    section 기획/세팅
    초기 세팅·구조 설계        :2026-05-24, 2d
    section 인증/마이그레이션
    Google OAuth 설계·구현     :2026-05-28, 2d
    Next.js 마이그레이션        :2026-05-31, 2d
    section 핵심 기능
    추천 알고리즘·대시보드·시간표 :2026-06-03, 2d
    section 고도화/QA
    내 시간표·경쟁률·모바일·테마 :2026-06-05, 3d
```
...
| 일자 | 커밋 수 | 주요 작업 |
|------|---------|----------|
| 05/24~05/25 | 12 | 프로젝트 초기 세팅, 폴더 구조 설계, 기본 UI 골격 |
| 05/28~05/29 | 35 | Google OAuth 설계/구현, 인증 흐름 정립 |
| 05/31~06/01 | 16 | Next.js(App Router) 마이그레이션, 핵심 페이지 이전 |
| 06/03~06/04 | 44 | 졸업요건 계산·대시보드, 시간표 추천 알고리즘, 시간표 그리드 |
| 06/05~06/07 | 94 | 내 시간표(복사·저장·확정·다중), 경쟁률 트래커, 모바일 반응형, 다크모드/테마, QA·문서화 |
...
- **EF 세부영역**: 물리·화학·DL 각 4학점, **수학은 학번별(22~24학번 4 / 25학번~ 8)**
...
| 2 | EF 수학 필수 학점이 학번마다 다름(22~24=4, 25~=8) | `student_id` 앞 4자리(입학년도)로 요건을 동적 분기, 대시보드·추천·계산 일원 적용 |
