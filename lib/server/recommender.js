...
import { isGraduateCourse } from './parser.js';
...
async function getCompletedInfo(userId, supabase) {
  ...
  for (const row of (data || [])) {
    const { code, category, credits, grad_excluded } = row.courses || {};
    if (!code || seenCodes.has(code)) continue;
    seenCodes.add(code);
    codes.push(code);
    if (grad_excluded) continue; // 졸업학점 미포함 과목은 학점/요건 집계 제외
    if (isGraduateCourse(code, category) && row.grad_included === false) continue; // 대학원 과목: 사용자가 졸업학점 미포함으로 선택
...
function filterCourses(courses, userGrade, espNextCodes) {
  return courses.filter(c => {
    // 졸업학점 미포함 과목 제외
    if (c.grad_excluded) return false;
    // RC 체육 제외
    ...
    // CAPS는 target_grade(4학년) 미만 학생에게 추천 제외
    if (c.category === 'CAPS' && userGrade < c.target_grade) return false;
...
async function getRequiredCourses(userId, preferences = {}, semesterOverride = null) {
  const supabase = getSupabaseAdmin();

  const [completedInfo, semester, userRow] = await Promise.all([
    getCompletedInfo(userId, supabase),
    semesterOverride || getLatestSemester(supabase),
    supabase.from('users').select('grade, student_id').eq('id', userId).maybeSingle(),
  ]);
  ...
  // EF 수학 필수: 22·23·24학번 4 / 25학번 이후 8 (학번 앞 4자리 = 입학년도)
  const admitYear = parseInt(String(userRow?.data?.student_id || '').slice(0, 4));
  const efSubReq = { ...EF_SUB_REQ, math: (admitYear && admitYear < 2025) ? 4 : 8 };

  // [Fix15] 이미 필수 학점을 채운 EF 세부영역 → 해당 영역 과목은 추천 제외
  const fulfilledEfSubs = new Set(
    Object.entries(efSubReq)
  ...
  return { mandatory, elective, userGrade, elUpperCount, earnedByCat };
}
...
const COMPACT_WINDOW = 5; // compact 모드: 상위 N개 후보 중 공강 최소 과목 선택

// 주어진 시간대 목록의 하루 총 공강 시간(분) 계산
function totalDailyGap(slots) {
  const byDay = {};
...
function buildPlan(candidates, maxCredits = 21, minCredits = MIN_CREDITS, catLimits = null, compact = false) {
  ...
  const overLimit = (course) =>
    catLimits?.[course.category] != null &&
    (catCount[course.category] || 0) >= catLimits[course.category];

  const isFeasible = (course) =>
    !usedCodes.has(course.code) &&
    totalCredits + Number(course.credits) <= maxCredits &&
    !overLimit(course) &&
    !plan.some(c => hasTimeConflict(c, course));

  const addToPlan = (course) => {
    plan.push(course);
    usedCodes.add(course.code);
    catCount[course.category] = (catCount[course.category] || 0) + 1;
    totalCredits += Number(course.credits);
  };

  if (compact) {
    // compact 모드: 우선순위 상위 COMPACT_WINDOW개 중 공강 시간 최소 과목 선택
    ...
    while (remaining.length > 0) {
      ...
      for (const c of remaining) {
        if (isFeasible(c)) {
          window.push(c);
          if (window.length >= COMPACT_WINDOW) break;
        }
      }
      if (window.length === 0) break;

      const planSlots = plan.flatMap(c => c.timeslots || []);
      const chosen = window.reduce((best, c) => {
        const gapC = totalDailyGap([...planSlots, ...(c.timeslots || [])]);
        const gapB = totalDailyGap([...planSlots, ...(best.timeslots || [])]);
        return gapC < gapB ? c : best;
      });
    ...
    for (const course of sorted) {
      if (isFeasible(course)) addToPlan(course);
    }
  }

  // 최소 학점 미달 시 유학점 후보에서 추가 시도 (catLimits 완화)
  if (totalCredits < minCredits) {
    for (const course of sorted) {
      if (totalCredits >= minCredits) break;
      if (usedCodes.has(course.code)) continue;
      if (Number(course.credits) === 0) continue;
      if (totalCredits + Number(course.credits) > maxCredits) continue;
      if (!plan.some(c => hasTimeConflict(c, course))) addToPlan(course);
    }
  }
...
// 추천 정렬: 내 학년 → 공통 → 상위 학년
// 같은 학년 그룹 내: EL(트랙+학년 기반) > 비EL, EL끼리는 선호 트랙 순
function sortByGradeAndTrack(courses, userGrade, preferredTracks = []) {
  return [...courses].sort((a, b) => {
    ...
    // 같은 학년 그룹 내 카테고리 우선순위: EL(0) > ESP(1) > 나머지(2)
    // ESP는 수강 단계가 명확해 EF(학년 없음) 등보다 먼저 추천
    const catPri = c => c.category === 'EL' ? 0 : c.category === 'ESP' ? 1 : 2;
    const priA = catPri(a), priB = catPri(b);
    if (priA !== priB) return priA - priB;

    // EL끼리는 선호 트랙 순서로 세부 정렬
...
// 남은 요건 학점이 threshold 이하인 카테고리를 동적으로 우선 배치
// EL·ESP는 별도 로직, HASS·EN은 compact 최적화에 맡김
// CAPS는 filterCourses에서 target_grade 미만 학생에게 이미 제외됨
const BUMP_SKIP_CATS = new Set(['EL', 'ESP', 'HASS', 'EN', 'CAPS']);
const BUMP_THRESHOLD = 8; // 남은 학점이 이 이하면 앞으로

function bumpPriorityCats(courses, earnedByCat = {}) {
  const priorityCats = new Set(
    Object.entries(CAT_GRAD_REQ)
      .filter(([cat, req]) => {
        if (BUMP_SKIP_CATS.has(cat)) return false;
        const remaining = req - (earnedByCat[cat] || 0);
        return remaining > 0 && remaining <= BUMP_THRESHOLD;
      })
      .map(([cat]) => cat)
  );
  ...
  for (const c of courses) {
    if (priorityCats.has(c.category) && !seen.has(c.category)) {
      front.push(c);
      seen.add(c.category);
    } else {
      rest.push(c);
...
// [Fix19] 일반적인 수강 패턴: 한 플랜당 EL 최대 2개 · ESP 최대 1개
const CAT_LIMITS = { EL: 2, ESP: 1 };

export async function generateRecommendations(userId, semester, preferences = {}, n = 3, intent = null, maxCredits = 16) {
  const { mandatory, elective, userGrade, elUpperCount, earnedByCat } = await getRequiredCourses(userId, preferences, semester);

  let filteredMandatory = mandatory;
  let filteredElective  = elective;
  ...
  const compact = !!preferences.prefer_compact;

  const plans = [];
  for (let i = 0; i < n; i++) {
    const usedCodes = new Set(plans.flatMap(p => p.map(c => c.code)));

    // 1단계: 필수 영역 — plan마다 bumpPriorityCats 재적용
    ...
    const candidates = bumpPriorityCats([...freshMandatory, ...reusedMandatory], earnedByCat);
    const plan = buildPlan(candidates, maxCredits, minCredits, CAT_LIMITS, compact);
    ...
    if (remaining > 0) {
      ...
      const extra = buildPlan([...plan, ...freshElective], maxCredits, 0, CAT_LIMITS, compact)
