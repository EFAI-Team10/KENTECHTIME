...
async function getCompletedInfo(userId, supabase) {
  ...
  return { codes, earnedByCat, elUpperCount };
}
...
async function getRequiredCourses(userId, preferences = {}) {
  ...
  const { codes: completedCodes, earnedByCat, elUpperCount } = completedInfo;
...
// [Fix2] EL4/5 과목을 EL 후보 앞으로 배치 (elUpperCount < 2일 때만)
// 선호 트랙 EL4/5가 비선호 트랙 EL4/5보다 앞에 오도록 track 기준 재정렬
function bumpEL45(courses, elUpperCount, preferredTracks = []) {
  ...
  const upper = courses.filter(isUpper);
  ...
  return [...upper, ...courses.filter(c => !isUpper(c))];
}
...
export async function generateRecommendations(userId, _semester, preferences = {}, n = 3, intent = null, maxCredits = 21) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    const plan = buildPlan(candidates, maxCredits, minCredits);
