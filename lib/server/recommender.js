...
// 1학년 전용 과목 — 해당 학년 아닌 학생에게는 추천 제외
...
function filterByGrade(courses, userGrade) {
  return courses.filter(c => {
    // RC 체육은 추천 제외
    if (RC_EXCLUDED_CODES.has(c.code)) return false;

    // ESP·IR 등 시간표 없는 0학점 과목 추천 제외 (시간표에 표시 불가)
    ...
    // VC는 1학년 전용 — 2학년 이상에게 추천 제외
    if (c.category === 'VC' && userGrade > VC_ONLY_GRADE) return false;

    // 권장 학년이 사용자 학년보다 낮은 과목은 추천 제외
    // (이미 지나간 학년의 과목 — 기수강했어야 할 것)
...
async function getRequiredCourses(userId, preferences = {}) {
  ...
  const mandatory = filterByGrade(rawMandatory || [], userGrade);
  ...
  const elective = filterByGrade(rawElective || [], userGrade);
