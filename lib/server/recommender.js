...
function buildPlan(candidates, maxCredits = 21, minCredits = MIN_CREDITS, minGap = 0, catLimits = null) {
  ...
  const catCount = {}; // [Fix19] 카테고리별 담긴 개수
  let totalCredits = 0;

  // [Fix19] catLimits = { EL:2, ESP:1 } 같은 카테고리별 최대 개수
  ...
  for (const course of sorted) {
    ...
    if (overLimit(course)) continue; // [Fix19] 카테고리 상한 초과 제외
  ...
  if (totalCredits < minCredits) {
    for (const course of sorted) {
      ...
      if (totalCredits + Number(course.credits) > maxCredits) continue;
      // 최소학점 보충 단계에서는 카테고리 상한을 완화 (졸업 최소학점 규칙 우선)
...
export async function generateRecommendations(userId, _semester, preferences = {}, n = 3, intent = null, maxCredits = 16) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    const plan = buildPlan(candidates, maxCredits, minCredits, minGap, CAT_LIMITS);
    ...
    if (remaining > 0) {
      ...
      // [Fix19] EL/ESP 상한도 1·2단계 합산 적용 (plan을 seed로 넘겨 누적 카운트)
      const extra = buildPlan([...plan, ...freshElective], maxCredits, 0, minGap, CAT_LIMITS)
