...
function buildPlan(candidates, maxCredits = 21, minCredits = MIN_CREDITS, minGap = 0, catLimits = null, compact = false) {
  ...
  const isFeasible = (course) =>
    ...
    !overLimit(course) &&
    !plan.some(c => hasTimeConflict(c, course) || violatesMinGap(c, course, minGap));
...
export async function generateRecommendations(userId, _semester, preferences = {}, n = 3, intent = null, maxCredits = 16) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    const plan = buildPlan(candidates, maxCredits, minCredits, minGap, CAT_LIMITS, compact);
    ...
    if (remaining > 0) {
      ...
      const extra = buildPlan([...plan, ...freshElective], maxCredits, 0, minGap, CAT_LIMITS, compact)
