...
// 필수 졸업학점 영역 (GR·RC체육 제외, FR은 2순위에서 처리)
...
async function getRequiredCourses(userId, preferences = {}) {
  ...
  const [completedCodes, semester] = await Promise.all([
    getCompletedCodes(userId, supabase),
    getLatestSemester(supabase),
  ]);

  if (!semester) return { mandatory: [], elective: [] };
  ...
  // 1순위: 필수 영역 (VC/EF/EL/MN/HASS/ESP/IR/CAPS/EN/RC)
  const { data: mandatory } = await buildQuery(MANDATORY_CATS);

  // 2순위: 선호 자유 영역 (FR/MN/HASS/EN/EL) — preferences.elective_cats로 덮어씌울 수 있음
  const electiveCats = preferences.elective_cats?.length
    ? preferences.elective_cats
    : ELECTIVE_CATS;
  const { data: elective } = await buildQuery(electiveCats);
...
export async function generateRecommendations(userId, _semester, preferences = {}, n = 3, intent = null, maxCredits = 21) {
  const { mandatory, elective } = await getRequiredCourses(userId, preferences);
